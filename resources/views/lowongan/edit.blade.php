...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8">
        ...
        <p class="mt-2 text-sm text-gray-600">Perbarui informasi lowongan magang</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-8">
        ...
        <form method="POST" action="{{ route('lowongan.update', $lowongan) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                >
                    ...
                    @foreach($departemens as $departemen)
                        <option value="{{ $departemen->id }}" {{ (old('dept_id', $lowongan->dept_id) == $departemen->id) ? 'selected' : '' }}>
                            {{ $departemen->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input 
                    ...
                    required 
                    min="1"
                    value="{{ old('quota', $lowongan->quota) }}"
                    class="block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-150"
